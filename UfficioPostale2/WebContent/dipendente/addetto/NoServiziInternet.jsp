	...
	<div id="mainDiv">
		...
		%>		<div class="dialog">
					<header class="dialogTitle">Fatto!</header>
						...
						<div class="clear"></div>
						<a href="/UfficioPostale2/dipendente/addetto/NoServiziInternet.jsp">back</a>
						<div id="sfondoDiv"></div>
				</div>
		<%		
			}else{
				ClienteModel modelC=new ClienteModel();
				ArrayList<ContoCliente> array= modelC.cercaNoServiziInternet();
		...
		<%	
					...
					String tipo= "NoServiziInternetServlet";
		%>				<div id="scrittaElenco">Elenco dei clienti che hanno un conto senza servizi internet</div>
						<div id="descrizione">
							In questa sezione è possibile visualizzare un elenco dei clienti le cui funzionalità del sito non sono attivate.
							&Egrave; possibile invitare un cliente ad usufruire dei Servizi Internet, <br>
							attraverso l'azione "invia richiesta". &Egrave; possibile visualizzare ulteriori dettagli riguardo il cliente, 
							selezionando la voce "visualizza".
						</div>
						<table>
							<tr>
								<th>Nome</th>
		...
		%>				<tr>
							...
							<td>
								<form action="dettagli.jsp?tipo=NoSI" method="post">
									<input type="text" value="<%=cc.getIban()%>" hidden="true" name="iban">
									<input type="text" value="<%=cliente.getCf()%>" hidden="true" name="cf">

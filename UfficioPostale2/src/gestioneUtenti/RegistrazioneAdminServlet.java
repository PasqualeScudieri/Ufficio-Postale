...
public class RegistrazioneAdminServlet extends HttpServlet {
     ...
     */
    public RegistrazioneAdminServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(tipo == null || tipo.trim().equals("")) {
			errore+= "Inserisci il tipo <br/>"; 
		} else if(!tipo.equals("addettoSportello") && !tipo.equals("postino")){
			errore+="inserire un tipo valido <br/>";
		}else {
		...
		if(mat == null || mat.trim().equals("")) {
			...
			try {
				...
				request.setAttribute("matricola", mat);
			}catch (NumberFormatException e){
				errore+="formato della matricola non valido <br/>";
			}
		}
	...
	*/
		if(errore != null && !errore.equals("")) {
			...
			RequestDispatcher dispatcher= getServletContext().getRequestDispatcher("/gestore/registrazioneAdmin.jsp");
		...
		try{
			UtenteBean utente=modelU.doRetrieveByUser(matricola+"");
			...
			DipendentiBean dipendente=modelD.cercaByCf(cf);
			if(!dipendente.getCf().equals("")) {
				errore+="è già presente un dipendente con questo cf <br>";
				request.setAttribute("error", errore);
				RequestDispatcher dispatcher= getServletContext().getRequestDispatcher("/gestore/registrazioneAdmin.jsp");
				dispatcher.forward(request, response);
				return;
			...
			dipendente=modelD.cercaByMatricola(matricola);
			if(dipendente.getMatricola()!=0) {
				errore+="è già presente un dipendente con questa matricola <br>";
				request.setAttribute("error", errore);
				RequestDispatcher dispatcher= getServletContext().getRequestDispatcher("/gestore/registrazioneAdmin.jsp");
				dispatcher.forward(request, response);
				return;
		...
		DipendentiBean nuovoDipendente= new DipendentiBean();
		nuovoDipendente.setNome(nome);
		nuovoDipendente.setCognome(cognome);
		nuovoDipendente.setIndirizzo(indirizzo);
		nuovoDipendente.setLuogonascita(luogoNascita);
		nuovoDipendente.setCf(cf);
		...
		nuovoDipendente.setDataNascita(data);
		nuovoDipendente.setTipo(tipo);
		nuovoDipendente.setMatricola(matricola);
		try {
			modelD.inserisciDipendente(nuovoDipendente);
		}catch (SQLException e) {
			RequestDispatcher dispatcher= getServletContext().getRequestDispatcher("/error.jsp");
			dispatcher.forward(request, response);
		}
		...
		synchronized (session) {
			...
			session.setAttribute("dipendente", nuovoDipendente);
		...
		*/
		...
		request.setAttribute("nuovoDip", nuovoDipendente);
		
		/*mandare la mail**/
		try {
		SendMail.sendMailTLS(email, "completa registrazione", "per completare la tua registrazione vai all'indirizzo http://localhost:8080/UfficioPostale2/completaRegistrazioneDip.jsp?nome="+matricola);
		}catch (Exception e) {
			RequestDispatcher dispatcher= getServletContext().getRequestDispatcher("/error.jsp");
			dispatcher.forward(request, response);
		}
